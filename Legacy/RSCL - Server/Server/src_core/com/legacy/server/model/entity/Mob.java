...
import com.legacy.server.event.rsc.impl.StatRestorationEvent;
import com.legacy.server.event.rsc.impl.combat.CombatEvent;
...
import com.legacy.server.model.ViewArea;
import com.legacy.server.model.WalkingQueue;
import com.legacy.server.model.entity.npc.Npc;
import com.legacy.server.model.entity.player.Player;
import com.legacy.server.model.entity.update.Damage;
import com.legacy.server.model.entity.update.UpdateFlags;
import com.legacy.server.model.states.Action;
import com.legacy.server.model.states.CombatState;
import com.legacy.server.model.world.World;
import com.legacy.server.net.rsc.ActionSender;
import com.legacy.server.util.rsc.CollisionFlag;
...
public abstract class Mob extends Entity {
	...
	private long playerCombatTimer = 0;
	/**
	...
	private boolean canReach(int minX, int maxX, int minY, int maxY) {
		...
		if (minX <= getX() - 1 && maxX >= getX() - 1 && minY <= getY() && maxY >= getY()
				&& (World.mapValues[getX() - 1][getY()] & CollisionFlag.WALL_WEST) == 0) {
			return true;
		}
		if (1 + getX() >= minX && getX() + 1 <= maxX && getY() >= minY && maxY >= getY()
				&& (CollisionFlag.WALL_EAST & (World.mapValues[getX() + 1][getY()])) == 0) {
			return true;
		}
		if (minX <= getX() && maxX >= getX() && getY() - 1 >= minY && maxY >= getY() - 1
				&& (CollisionFlag.WALL_SOUTH & (World.mapValues[getX()][getY() - 1])) == 0) {
			return true;
		}
		if (minX <= getX() && getX() <= maxX && minY <= getY() + 1 && maxY >= getY() + 1
				&& (CollisionFlag.WALL_NORTH & (World.mapValues[getX()][getY() + 1])) == 0) {
			return true;
		}
		return false;
	}
	...
	private boolean isMapBlocking(Entity e, int x, int y, int bit) {
		int val = World.mapValues[x][y];
		if ((val & bit) != 0) {
			return true;
		}
		if ((val & 16) != 0) {
	...
	private boolean isObjectBlocking(Entity e, int x, int y, byte bit) {
		...
		if ((val & 32) != 0 && !Formulae.doorAtFacing(e, x, y, 3) && !Formulae.objectAtFacing(e, x, y, 1)) // There is a diagonal wall here: /
			return true;
		if ((val & 64) != 0 && (e instanceof Npc || e instanceof Player || (e instanceof GroundItem && !((GroundItem)e).isOn(x, y)) || (e instanceof GameObject && !((GameObject)e).isOn(x, y)))) // There is an object here, doesn't block items (ontop of it) or the object itself though
			return true;
		return false;
	}
	...
	public long getPlayerCombatTimer() {
		return playerCombatTimer;
	}
	...
	public void setPlayerCombatTimer() {
		playerCombatTimer = System.currentTimeMillis();
	}
	...
	public void startCombat(Mob victim) {
		...
		if (victim.isPlayer()) {
			...
			if (this.isPlayer()) {
				...
				((Player) this).setPlayerCombatTimer();
			}
			playerVictim.resetAll();
			playerVictim.checkAndInterruptBatchEvent();
			playerVictim.setPlayerCombatTimer();
