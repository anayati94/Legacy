...
public class Inventory {
	...
	public void add(Item itemToAdd, boolean sendInventory) {
		synchronized (list) {
			...
			if (this.full()) {
				...
				world.registerItem(new GroundItem(itemToAdd.getID(), player.getX(), player.getY(), itemToAdd.getAmount(), player));
	...
	public void wieldItem(Item item, boolean sound) {
		...
		if ((item.getID() == 401 || item.getID() == 407)
				&& (player.getQuestStage(Constants.Quests.DRAGON_SLAYER) != -1 && !player.getCache().hasKey("rune_plate_card"))) {
			player.message("you have not earned the right to wear this yet");
			player.message("you need to complete the dragon slayer quest");
			player.message("or activate the Rune Plate Body Card");
			return;
		}
		if (item.getID() == 593 && (player.getQuestStage(Constants.Quests.LOST_CITY) != -1 && !player.getCache().hasKey("dragon_sword_card"))) {
			player.message("you have not earned the right to wear this yet");
			player.message("you need to complete the Lost city of zanaris quest");
			player.message("or activate the Dragon Sword Card");
			return;
		}
		if (item.getID() == 594 && ((player.getQuestStage(Constants.Quests.HEROS_QUEST) != -1 || player.getQuestPoints() < 56) && !player.getCache().hasKey("dragon_axe_card")) ) {
			player.message("you have not earned the right to wear this yet");
			player.message("you need to complete the Hero's guild entry quest");
			player.message("or activate the Dragon Axe Card");
			return;
		}
		/*
		...
		if((item.getID() == 2254 || item.getID() == 1288) && player.getQuestStage(Constants.Quests.LEGENDS_QUEST) != -1) {
			player.message("you have not earned the right to wear this yet");
			player.message("you need to complete the Legends Quest");
			return;
		}
		if(item.getID() == 1278 && (player.getQuestStage(Constants.Quests.LEGENDS_QUEST) != -1 && !player.getCache().hasKey("dragon_sq_card"))) {
			player.message("you have not earned the right to wear this yet");
			player.message("you need to complete the Legends Quest");
			player.message("or activate the Dragon Square Shield Card");
			return;
		}
	...
	public void dropOnDeath(Mob opponent) {
		...
		DeathLog log = new DeathLog(player, opponent, false, player.getCurrentIP());
		for (; iterator.hasNext();) {
			...
			/**
			 * ALWAYS KEEP THE PUMPKIN HEADS AND SPOTTED CAPE IF YOU DIE
			 **/
			if ((item.getID() >= 2097 && item.getID() <= 2102) || item.getID() == 2110) {
				continue;
			}
			if (item.isWielded()) {
