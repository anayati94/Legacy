...
import com.legacy.server.event.rsc.impl.FireCannonEvent;
import com.legacy.server.event.rsc.impl.PoisonEvent;
import com.legacy.server.event.rsc.impl.PrayerDrainEvent;
...
import com.legacy.server.model.action.WalkToAction;
...
import com.legacy.server.net.rsc.PacketHandlerLookup;
...
public final class Player extends Mob {
	...
	public boolean inClanWars = false;
	
	
	public void setInClanWars(boolean inClanWars) {
		this.inClanWars = inClanWars;
	}
	
	public boolean isInClanWars() {
		return inClanWars;
	}
	...
	private long consumeTimer, rangeTimer = 0;
	...
	public boolean cantRange() {
		return rangeTimer - System.currentTimeMillis() > 0;
	}

	public void setRangeTimer(long l) {
		rangeTimer = System.currentTimeMillis() + l;
	}
	...
	public double getExperienceRate(int skill) {
		 ...
		 **/
		else if (skill >= 0 && skill <= 6) { // Attack, Strength, Defense, HP, Prayer, Magic, Ranged.
	...
	public boolean isMod() {
		return groupID == 2 || isAdmin() || groupID == 7 || groupID == 3 || groupID == 5;
	}
	public boolean isEventMod() {
		return groupID == 5;
	}
	...
	public void killedBy(Mob mob) {
		...
		boolean stake = getDuel().isDuelActive() || (player != null && player.getDuel().isDuelActive()) || (player != null && !player.getLocation().inWilderness());
		...
		if (player != null) {
			...
			if (player.getLocation().inWilderness()) {
				...
				if(!player.getCurrentIP().equals(this.getCurrentIP()) && player.isSkulled() && this.isSkulled()) {
					player.incKills();
					GameLogging.addQuery(new LiveFeedLog(player, "has PKed <strong>" + this.getUsername() + "</strong> in wilderness level: " + player.getLocation().wildernessLevel() + ", 1 kill point gained!"));
				} else {
					GameLogging.addQuery(new LiveFeedLog(player, "has PKed <strong>" + this.getUsername() + "</strong> in wilderness level: " + player.getLocation().wildernessLevel() + "!"));
				}
				this.incDeaths();
			} else if (stake) {
				GameLogging.addQuery(new LiveFeedLog(player, "has just won a stake against <strong>" + this.getUsername() + "</strong>"));
			}
		}
		if (stake) {
			getDuel().dropOnDeath();
	...
	private long premiumSubscriptionExpires, combatSubscriptionExpires;
	...
	public int combatSubscriberSubDaysLeft() {
		long now = (System.currentTimeMillis() / 1000);
		if (combatSubscriptionExpires == 0 || now >= combatSubscriptionExpires) {
			return 0;
		}
		double days = (double) (combatSubscriptionExpires - now) / (double) 86400;
		if (days > 0.0 && days < 1.0) {
			return 1;
		}
		return (int) Math.round(days);
	...
	public boolean isCombatSubscriber() {
		return combatSubscriberSubDaysLeft() > 0;
	}
	...
	public long getCombatSubExpires() {
		return combatSubscriptionExpires;
	}

	public void setCombatSubExpires(long long1) {
		this.combatSubscriptionExpires = long1;
	}
