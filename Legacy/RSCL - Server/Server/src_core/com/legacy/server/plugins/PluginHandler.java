...
import com.legacy.server.event.custom.ShopRestockEvent;
...
public final class PluginHandler {
					...
					if (queue.containsKey(interfce)) {
						for (final Class<?> clz : queue.values()) {
							if (clz.getName().equalsIgnoreCase(
									c.getClass().getName())) {
								go = true;
								if(Constants.GameServer.DEVELOPMENT_PLATFORM)
	...
	public void initPlugins() {
		try {
			...
			for (final Class<?> interfce : loadInterfaces("com.legacy.server.plugins.listeners.action")) {
			...
			for (final Class<?> interfce : loadInterfaces("com.legacy.server.plugins.listeners.executive")) {
	...
	public void unload() {
		try {
			...
			LOGGER.info("Plugins unload completed.");
		} catch(IOException e) {
			LOGGER.error(e);
		}
	}
