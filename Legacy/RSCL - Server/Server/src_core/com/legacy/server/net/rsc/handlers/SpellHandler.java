...
import com.legacy.server.event.rsc.impl.CustomProjectileEvent;
...
import com.legacy.server.model.TrajectoryHandler;
...
public class SpellHandler implements PacketHandler {
	public void handlePacket(Packet p, Player player) throws Exception {
		...
		if (pID == CAST_ON_SELF) { // Cast on self!
			...
			if (spell.getSpellType() == 1 || spell.getSpellType() == 2) {
				...
				if(affectedPlayer.getLocation().inBounds(220, 108, 225, 111) || player.getLocation().inBounds(220, 108, 225, 111)) { // mage arena block real rsc.
	...
	private void handleItemCast(Player player, final SpellDef spell, final int id, final GroundItem affectedItem) {
		...
		player.setWalkToAction(new WalkToPointAction(player, affectedItem.getLocation(), 4) {
			public void execute() {
				...
				if (TrajectoryHandler.isRangedBlocked(player.getX(), player.getY(), affectedItem.getX(), affectedItem.getY())) {
	...
	private void handleMobCast(final Player player, final Mob affectedMob, final int spellID) {
		...
		if (TrajectoryHandler.isRangedBlocked(player.getX(), player.getY(), affectedMob.getX(), affectedMob.getY())) {
		...
		if (affectedMob.isPlayer()) {
			...
			if (player.getLocation().inWilderness() && System.currentTimeMillis() - other.getLastRun() < 600) {
				player.resetPath();
				return;
			}
		}
		if (player.getLocation().inWilderness() && System.currentTimeMillis() - player.getLastRun() < 600) {
			player.resetPath();
			return;
		}
