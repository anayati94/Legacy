...
import com.legacy.server.sql.query.logs.DuelLog;
			...
			if (affectedPlayer.getDuel().isDuelConfirmAccepted()) {
				...
				player.setWalkToAction(new WalkToMobAction(player, affectedPlayer, 0) {
					public void execute() {
						...
						GameLogging.addQuery(new DuelLog(player.getUsername(), player.getCurrentIP(), affectedPlayer.getUsername(), affectedPlayer.getCurrentIP(), player.getDuel().getDuelOptions(), player.getDuel().getDuelOffer().getItems(), affectedPlayer.getDuel().getDuelOffer().getItems()).build());
