...
import com.legacy.server.model.entity.update.Projectile;
...
import com.legacy.server.sql.query.logs.PMLog;
...
public final class GameStateUpdater {
			...
			if (pm != null) {
				...
				if (affectedPlayer != null) {
					if ((affectedPlayer.getSocial().isFriendsWith(p.getUsernameHash()) || !affectedPlayer.getSettings()
							...
							&& !affectedPlayer.getSocial().isIgnoring(p.getUsernameHash()) || p.isMod() || p.isEventMod()) {
	...
	private static void updateTimeouts(Player player) {
		...
		if (player.isCombatSubscriber()) {
			timeoutLimit += 300000 * 3;
		}
	...
	public static void updateNpcs(Player playerToUpdate) throws Exception {
		com.legacy.server.net.PacketBuilder packet = new com.legacy.server.net.PacketBuilder();
		packet.setID(79);
	...
	public static void updatePlayers(Player playerToUpdate) throws Exception {
		...
		com.legacy.server.net.PacketBuilder positionBuilder = new com.legacy.server.net.PacketBuilder();
