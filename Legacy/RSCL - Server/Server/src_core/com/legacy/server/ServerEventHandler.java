...
public final class ServerEventHandler {
	...
	public void doEvents() {
		...
		/*events.removeIf(event -> event == null || event.shouldRemove() || event.getOwner() != null && event.getOwner().isUnregistering());
		
		events.parallelStream().filter(event -> event.shouldRun()).forEach(event -> {
			try {
				event.run();
				event.updateLastRun();
			} catch(Exception e) {
