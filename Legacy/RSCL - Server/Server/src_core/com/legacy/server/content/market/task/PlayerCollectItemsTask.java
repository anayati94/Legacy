...
public class PlayerCollectItemsTask extends MarketTask {
	...
	public void doTask() throws Exception {
		...
		try (Connection connection = MarketDatabase.con.getConnection();
				PreparedStatement setCollected = connection.prepareStatement(
						"UPDATE `" + Constants.GameServer.MYSQL_TABLE_PREFIX + "expired_auctions` SET `claim_time`= '"
								+ System.currentTimeMillis() + "',`claimed`='1' WHERE `claim_id`=?")) {
