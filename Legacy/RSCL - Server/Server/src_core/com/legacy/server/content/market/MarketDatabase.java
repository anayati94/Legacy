...
public class MarketDatabase {
	...
	public final static DatabaseConnection con = DatabaseConnection.getDatabase();
	...
	public static int getAuctionCount() {
		try (Connection connection = con.getConnection();
				PreparedStatement statement = connection.prepareStatement("SELECT count(*) as auction_count FROM `"
						+ Constants.GameServer.MYSQL_TABLE_PREFIX + "auctions` WHERE `sold-out`='0'")) {
	...
	public static MarketItem getAuctionItem(int auctionID) {
		try (Connection connection = con.getConnection();
				PreparedStatement statement = connection.prepareStatement(
						"SELECT `auctionID`, `itemID`, `amount`, `amount_left`, `price`, `seller`, `seller_username`, `buyer_info`, `time` FROM `"
								+ Constants.GameServer.MYSQL_TABLE_PREFIX
			...
			try (ResultSet result = statement.executeQuery()) {
				...
				MarketItem auctionItem = new MarketItem(result.getInt("auctionID"), result.getInt("itemID"),
						...
						result.getInt("seller"), result.getString("seller_username"), result.getString("buyer_info"),
						result.getLong("time"));
	...
	public static ArrayList<CollectableItem> getCollectableItemsFor(int player) {
		...
		try (Connection connection = con.getConnection();
				PreparedStatement statement = connection.prepareStatement(
						"SELECT `claim_id`, `item_id`, `item_amount`, `playerID`, `explanation` FROM `"
								+ Constants.GameServer.MYSQL_TABLE_PREFIX
