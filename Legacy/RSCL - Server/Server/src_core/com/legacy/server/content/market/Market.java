...
public class Market implements Runnable {
	...
	private final ScheduledExecutorService scheduledExecutor = Executors
			.newSingleThreadScheduledExecutor(new ThreadFactoryBuilder().setNameFormat("AuctionHouseService").build());
	...
	private void checkAndRemoveExpiredItems() {
		try {
			...
			if (expiredItems.size() != 0) {
				try (Connection connection = MarketDatabase.con.getConnection();
						PreparedStatement expiredItemsStatement = connection.prepareStatement("INSERT INTO `"
					...
					for (MarketItem expiredItem : expiredItems) {
						...
						if (sellerPlayer != null) {
							sellerPlayer.message("@gre@[Auction House] @whi@Your auction - @lre@" + def.getName() + " x"
									+ amount + "@whi@ has expired!");
	...
	public void shutdown() {
		...
		LOGGER.info("MarketExecutor scheduler shutdown completed.");
	}
}
