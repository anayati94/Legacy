...
public class RangeEvent extends GameTickEvent {
	...
	private boolean SHOT_LESARKUS = false;
		...
		if (!canReach(target)) {
			...
			if (canShoot) {
				...
				for (int aID : (xbow ? Formulae.boltIDs : Formulae.arrowIDs)) {
					...
					arrowID = aID;
					if (target.getID() == 361 && getPlayerOwner().getInventory().countId(723) < 1 && !SHOT_LESARKUS) {
						getPlayerOwner().message("You need to kill the fire warrior with ice arrows");
						getPlayerOwner().resetRange();
						return;
					}
