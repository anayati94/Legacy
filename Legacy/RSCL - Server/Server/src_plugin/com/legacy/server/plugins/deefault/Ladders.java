	...
	public void onObjectAction(GameObject obj, String command, Player player) {
		...
		if(obj.getID() == 79 && obj.getX() == 243 && obj.getY() == 95) {
			while (System.currentTimeMillis()
					- player.getPlayerCombatTimer() < 10000
					&& player.getLocation().inWilderness()) {
				player.message("You must be out of combat for 10 seconds before using the ladder");
				return;
			}
		...
		if(obj.getID() == 6 && (obj.getX() == 264 && obj.getY() == 361)) {
			while (System.currentTimeMillis()
					- player.getPlayerCombatTimer() < 10000
					&& player.getLocation().inWilderness()) {
				player.message("You must be out of pvp combat for 10 seconds before using the ladder");
				return;
			}
			player.teleport(262, 3188);
			player.message("You climb down the ladder");
			return;
		}
		if(obj.getID() == 5 && (obj.getX() == 263 && obj.getY() == 3188)) {
			while (System.currentTimeMillis()
					- player.getPlayerCombatTimer() < 10000
					&& player.getLocation().inWilderness()) {
				player.message("You must be out of pvp combat for 10 seconds before using the ladder");
		...
		if(obj.getID() == 1188) {
			player.teleport(446, 3368);
			player.message("You climb down the ladder");
			return;
		}
		if(obj.getID() == 1187) {
			player.teleport(222, 110);
			player.message("You climb up the ladder");
			return;
		}
		if(obj.getID() == 630) {
			player.teleport(576, 3528);
