...
public final class RegularPlayer implements CommandListener {
	...
	public void onCommand(String command, String[] args, Player player) {
		...
		/*if (command.equalsIgnoreCase("joinwar")) {
			if (player.getClan() == null) {
				player.message("You aren't in a clan");
				return;
			}
			int aMinX = 600, aMaxX = 614, aMinY = 700, aMaxY = 711; 
			player.teleport(DataConversions.random(aMinX, aMaxX),
					DataConversions.random(aMinY, aMaxY));
			player.setInClanWars(true);
			//player.getClanWarSession().getPlayerListFromClans().put(player, player.getClan().getClanName());
		}*/
		...
		if (command.equalsIgnoreCase("info")) {
			...
			+ "@gre@Combat sub Expires:@whi@ " + player.combatSubscriberSubDaysLeft() + " days % %"
			+ "@lre@Experience Rates:%"
			+ "@gre@Attack, Strength, Defense, Hits:@whi@ " + player.getExperienceRate(0) + "x %"
			+ "@gre@Ranged, Magic:@whi@ " + player.getExperienceRate(4) + "x %"
			+ "@gre@Prayer:@whi@ " + player.getExperienceRate(5) + "x %"
		...
		if (command.equals("g") || command.equals("p")) {
			...
			if (player.getLocation().onTutorialIsland() && (!player.isMod() || !player.isEventMod())) {
				return;
			}
			...
			for (Player p : World.getWorld().getPlayers()) {
				...
				if (p.getGlobalBlock() != 2) {
					...
					ActionSender.sendMessage(p, player, 1, MessageType.GLOBAL_CHAT, channelPrefix + "@whi@" + (player.getClan() != null ? "@cla@<" + player.getClan().getClanTag() + "> @whi@" : "") + header + player.getUsername() + ": "
							+ (channel == 1 ? "@gr2@" : "@or1@") + ChatFilter.censor(newStr), player.getIcon());
				}
			}
			if(command.equals("g")) {
		...
		if (command.equals("online")) {
			...
			for (Player p : World.getWorld().getPlayers()) {
				if ((p.isMod() ||  p.isEventMod()) && p.getSettings().getPrivacySetting(1)) {
					players--;
				}
			}
