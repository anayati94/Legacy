...
public class DoorAction {
	...
	public void onWallObjectAction(final GameObject obj, final Integer click,
		...
		case 160:
			if(obj.getX() == 702) {
				p.teleport(415, 2051);
			} else if(obj.getX() == 415) {
				p.teleport(701, 2338);
			}
			break;
		case 155: // grandtree doors first floor WEST
			if(obj.getX() == 419) {
				p.teleport(708, 1395);
			} else if(obj.getX() == 707) {
				p.teleport(418, 1107);
			}
			break;
		case 156: // grandtree doors first EAST
			if(obj.getX() == 414) {
				p.teleport(700, 1395);
			} else if(obj.getX() == 701) {
				p.teleport(414, 1107);
			}
			break;
		...
		case 1:
			...
			p.setBusyTimer(650);
			replaceObject(obj, new GameObject(obj.getLocation(), 2, obj.getDirection(), obj.getType()));
			p.playSound("closedoor");
			p.message("The door creaks shut");
			break;
		case 2:
			if(click == 1) {
				p.message("Nothing interesting happens");
			...
			p.setBusyTimer(650);
			replaceObject(obj, new GameObject(obj.getLocation(), 1, obj.getDirection(), obj.getType()));
