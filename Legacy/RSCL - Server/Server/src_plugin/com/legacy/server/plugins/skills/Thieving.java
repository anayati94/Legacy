...
WallObjectActionExecutiveListener, WallObjectActionListener {
	...
	public void handleChestThieving(Player player, GameObject obj) {
		...
		GameObject nonLootable = new GameObject(obj.getLocation(), 340, obj.getDirection(), obj.getType());
		replaceObject(obj, nonLootable);
		message(player, 1300, "You find a trap on the chest",
	...
	public void onObjectAction(GameObject obj, String command, Player player) {
		...
		if (formattedName.contains("STALL")) {
			...
			if (command.equalsIgnoreCase("Open")) {
				...
				World.getWorld().replaceGameObject(obj,new GameObject(obj.getLocation(), 340, obj.getDirection(), obj.getType()));
	...
	private boolean canBeSeen(int fromX, int fromY, int targetX, int targetY) {
		...
		while (!stop) {
			...
			if ((World.objectValues[fromX][fromY] & 64) != 0) {
				stop = true;
				return false;
			}
