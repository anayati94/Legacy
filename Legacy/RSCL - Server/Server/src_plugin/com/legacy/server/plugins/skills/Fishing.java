...
public class Fishing implements ObjectActionListener, ObjectActionExecutiveListener {
	...
	private void handleFishing(final GameObject object, Player owner, final int click, final String command) {
		...
		ObjectFishingDef defz = EntityHandler.getObjectFishingDef(object.getID(), click);
		if (object.getX() == 585 && object.getY() == 507 && object.getID() == 376) {
			defz = new ObjectFishingDef(376, 379, -1);
			defz.getFishDefs().add(new ObjectFishDef(369, 80, 100));
		}
		
		final ObjectFishingDef def = defz;
		...
		if (owner.getSkills().getLevel(10) < def.getReqLevel()) {
			owner.playerServerMessage(MessageType.QUEST, "You need at least level " + def.getReqLevel() + " "
					...
					+ (def.getFishDefs().size() > 1 ? "these fish"
							: EntityHandler.getItemDef(def.getFishDefs().get(0).getId()).getName().toLowerCase()
							.substring(4) + "s"));
			return;
		}
		...
		owner.setBatchEvent(new BatchEvent(owner, (object.getX() == 585 && object.getY() == 507 && object.getID() == 376 ? 800 : 1900), Formulae.getRepeatTimes(owner, 10)) {
			@Override
			public void action() {
				showBubble(owner, new Item(netId));
				final int baitId = def.getBaitId();
