...
InvUseOnObjectExecutiveListener {
	...
	public void onInvUseOnItem(Player player, Item item1, Item item2) {
		if (item1.getID() == 621 && doGlassBlowing(player, item1, item2)) {
			return;
			...
			public void action() {
				...
				if (player.getInventory().remove(gem) > -1) {
					...
					if((gem.getID() == 889 || gem.getID() == 890 || gem.getID() == 891) && DataConversions.random(0, 3) == 2) {
						player.message("You miss hit the chisel and smash the " + cutGem.getDef().getName() + " to pieces!");
						player.getInventory().add(new Item(915));
						player.incExp(12, (gem.getID() == 889 ? 6.25 : gem.getID() == 890 ? 5 : 3.75), true);
					} else {
						player.message("You cut the " + cutGem.getDef().getName().toLowerCase());
						player.playSound("chisel");
						player.getInventory().add(cutGem);
						player.incExp(12, gemDef.getExp(), true);
					}
				} else 
					interrupt();
			}
