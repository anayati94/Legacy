		...
		if((item.getID() == GOLD_TOKEN || item.getID() == PREMIUM_TOKEN)) {
			...
			if(menu == 0) {
				...
				GameLogging.addQuery(
						new GenericLog(player.getUsername(), "used " + item.getDef().getName() + " on " + otherPlayer.getUsername()
								+ " at coords " + otherPlayer.getLocation().toString() + " while standing at " + player.getLocation().toString()));
			}
		}
	}
}
