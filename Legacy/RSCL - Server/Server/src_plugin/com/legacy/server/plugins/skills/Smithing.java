		...
		case 177: /* Fixed dorics anvil. */
			if (player.getQuestStage(3) > -1) {
				Npc doric = World.getWorld().getNpc(144, 323, 327, 487, 492,
						true);
				doric.getUpdateFlags().setChatMessage(new ChatMessage(doric, "Heh who said you could use that?", player));
		...
		case 50: // Anvil
			...
			if(item.getID() == 1276) {
				if (player.getInventory().countId(168) < 1) {
					player.message("You need a hammer to fix the ancient shield.");
					return;
				}
				if(player.getQuestStage(Constants.Quests.LEGENDS_QUEST) != -1) {
					player.message("You need to complete Legends Quest first.");
					return;
				}
				if(!hasItem(player, 1277)) {
					player.message("You need both halves to make the ancient shield.");
					return;
				}
				if(player.getSkills().getLevel(13) < 60) {
					player.message("You need at least level 60 smithing to fix the ancient shield");
					return;
				}
				message(player, 1300, "You set to work trying to fix the ancient shield.",
						"It's seen some heavy action and needs some serious work doing to it.",
						"Even for an experienced armourer it is not an easy task",
						"but eventually it is ready.",
						"You have restored the dragon square shield to its former glory.");
				player.incExp(SMITHING, 60.0, true);
				removeItem(player, 1277, 1);
				removeItem(player, 1276, 1);
				addItem(player, 1278, 1);
				return;
			}
