...
ObjectActionExecutiveListener, InvUseOnObjectListener, InvUseOnObjectExecutiveListener {
	...
	public static final int YANILLE_CLIMBING_ROCKS = 1029;
	
	public static final int CATHERBY_STONE = 707;
	public static final int GRAB_VINE = 1189;

	@Override
	public boolean blockObjectAction(GameObject obj, String command,
			Player player) {
		if(inArray(obj.getID(), SHORTCUT_YANILLE_PIPE,
		...
		if(obj.getID() == CATHERBY_STONE && (obj.getX() == 421 || obj.getX() == 416)) {
			return true;
		}
		return false;
	...
	public void onObjectAction(GameObject obj, String command, Player p) {
		...
		case SHORTCUT_YANILLE_PIPE:
			...
			if(obj.getY() == 3352) {
				if (getCurrentLevel(p, AGILITY) < 70) {
					p.message("You need an agility level of 70 to attempt to squeeze through the pipe");
					p.setBusy(false);
					return;
				}
				message(p, 600, "You squeeze through the pipe");
				if(p.getX() >= 375) {
					movePlayer(p, obj.getX() - 2, 3352);
				} else {
					movePlayer(p, obj.getX() + 2, 3352);
				}
		...
		case GREW_ISLAND_ROPE_ATTACHED:
			if(obj.getX() == 106 || obj.getX() == 102) {
				if (getCurrentLevel(p, AGILITY) < 25) {
					p.message("You need an agility level of 25 to attempt to swing across the stream");
					p.setBusy(false);
					return;
				}
				message(p, 1300, "You take the rope and swing");
				movePlayer(p, 104, 688);
				if(succeed(p, 25)) {
					message(p, 2000, "You land carefully on a stone");
					if(obj.getX() == 106) {
						movePlayer(p, 102, 688);
					} else {
						movePlayer(p, 106, 687);
					}
					p.message("You jump over to the other side");
					p.incExp(AGILITY, 10, true);
				} else {
					message(p, 2000, "You land on a stone");
					if(obj.getX() == 106) {
						message(p, 1200, "You loose your balance and fall in the stream");
						movePlayer(p, 109, 704);
						p.message("you manage to hold onto a rock and pull yourself up");
					} else {
						message(p, 1200, "You fall of the stone - the stream very is strong");
						movePlayer(p, 100, 704);
						p.message("You float up at the end of the river");
					}
				}
			} 
			else if(obj.getX() == 102 && obj.getY() == 689) {
				if (getCurrentLevel(p, AGILITY) < 25) {
					p.message("You need an agility level of 25 to attempt to swing across the stream");
					p.setBusy(false);
					return;
		...
		case EAST_KARAMJA_STONES:
			...
			message(p, 600, "You jump onto the rock");
			/**
			 * ARDOUGNE NEW SHORTCUT
			 */
			if(obj.getY() == 572) {
				movePlayer(p, obj.getX(), obj.getY());
				if(!succeed(p, 32)) {
					p.message("@red@!!! You Fall !!!");
					p.teleport(597, 577);
					sleep(800);
					p.teleport(598, 582);
					message(p, 1300, "You get washed up further down the river...");
					p.damage((int) (p.getSkills().getLevel(3) / 4) + 2);
					p.setBusy(false);
				} else {
					if(p.getX() <= 596) {
						movePlayer(p, 598, 572);
					} else {
						movePlayer(p, 595, 572);
					}
		...
		case YANILLE_CLIMBING_ROCKS:
			if(p.getFatigue() >= 7500) {
				p.message("You are too tired to climb up the rocks");
				p.setBusy(false);
				return;
			}
			if (getCurrentLevel(p, AGILITY) < 15) {
				p.message("You need an agility level of 15 to climb up the rocks");
				p.setBusy(false);
				return;
			}
			p.message("You climb up over the wall");
			p.teleport(624, 740);
			p.incExp(AGILITY, 10.0, true);
			break;
		case CATHERBY_STONE:
			if (getCurrentLevel(p, AGILITY) < 82) {
				p.message("You need an agility level of 82 to step these stones");
				p.setBusy(false);
				return;
			}
			...
			sleep(1000);
			if(obj.getX() == 421) {
				movePlayer(p, 420, 502);
				playerTalk(p, null, "Hup!");
				if(!succeed(p, 82)) {
					message(p, 1200, "You loose your footing");
					movePlayer(p, 425, 501);
					playerTalk(p, null, "Aeeeehearh!");
					movePlayer(p, 431, 499);
					p.message("You get washed up further down on land...");
					return;
				}
				movePlayer(p, 418, 503);
				playerTalk(p, null, "woooOo!");
				movePlayer(p, 416, 504);
				p.message("and walk across");
				p.incExp(AGILITY, 40.0, true);
			} else if(obj.getX() == 416) {
				movePlayer(p, 418, 503);
				playerTalk(p, null, "Hup!");
				if(!succeed(p, 82)) {
					message(p, 1200, "You slip from the stone");
					movePlayer(p, 418, 508);
					playerTalk(p, null, "ahhhhhrggh!");
					movePlayer(p, 416, 511);
					p.message("You manage to crawl up to land");
					return;
				}
				movePlayer(p, 420, 502);
				playerTalk(p, null, "woooOo!");
				movePlayer(p, 421, 500);
				p.message("and walk across");
				p.incExp(AGILITY, 40.0, true);
			}
			break;
		case GRAB_VINE:
			if (getCurrentLevel(p, AGILITY) < 45) {
				p.message("You need an agility level of 45 to step these stones");
				p.setBusy(false);
				return;
			}
			movePlayer(p, obj.getX(), obj.getY());
			message(p, 1300, "You grab the vine which sticks out from the mountain");
			movePlayer(p, 398, 497);
			p.message("You drop down to the ground");
			break;
		}
		p.setBusy(false);
	}
