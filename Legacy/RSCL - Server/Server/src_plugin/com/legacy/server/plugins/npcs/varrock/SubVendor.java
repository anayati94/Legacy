...
public class SubVendor implements TalkToNpcExecutiveListener, TalkToNpcListener, PickupListener, PickupExecutiveListener, InvActionListener, InvActionExecutiveListener, InvUseOnItemListener, InvUseOnItemExecutiveListener {
	...
	private final int COMBAT_TOKEN = 2096;
	...
	public void onTalkToNpc(Player p, Npc n) {
		if(n.getID() == SUB_VENDOR_FEMALE || n.getID() == SUB_VENDOR_MALE) {
			npcTalk(p, n, "Hello, I'm the subscription vendor",
					"How can I help you?");
			int menu = showMenu(p, n, "I want to exchange my gold token", "I want to exchange my premium token", "I want to exchange my combat token", "What are these tokens?", "I want to claim my teleport stones", "I want to change my death spawn location", "I want to reset my K/D Ratio");
			if(menu == 0) {
				...
				if(hasItem(p, COMBAT_TOKEN)) {
					tokenExchangeTime(p, n, new Item(COMBAT_TOKEN));
				} else {
					p.message("You don't have any combat tokens");
				}
			} else if(menu == 3) {
				...
				npcTalk(p, n, "One reset of your KD Ratio costs a token", "Which token would you like to trade in?");
				int tokenMenu = showMenu(p, n, "I want to reset with a Gold Token", "I want to reset with a Premium Token");
				if(tokenMenu == 0) {
					if(hasItem(p, GOLD_TOKEN)) {
						removeItem(p, GOLD_TOKEN, 1);
						p.message("Your KD Ratio has been reset");
						p.setKills(0);
						p.setDeaths(0);
					} else {
						playerTalk(p, n, "Oops I don't have a gold token with me");
					}
				} else if(tokenMenu == 1) {
					if(hasItem(p, PREMIUM_TOKEN)) {
						removeItem(p, PREMIUM_TOKEN, 1);
						p.message("Your KD Ratio has been reset");
						p.setKills(0);
						p.setDeaths(0);
					} else {
						playerTalk(p, n, "Oops I don't have a premium token with me");
					}
				}
			}
		}
		...
		if(i.getID() == GOLD_TOKEN && i.getX() == 119 && i.getY() == 516) {
			ActionSender.sendBox(p, 
					"@yel@Subscription Token %"
							...
							+ "www.rsclegacy.com/donate.php" , true);
		}
	}
	...
	private void tokenExchangeTime(Player p, Npc n, Item i) {
		...
		case GOLD_TOKEN:
			if(removeItem(p, i.getID(), 1)) {
				...
				try (Connection connection = DatabaseConnection.getDatabase().getConnection();
						PreparedStatement statement = connection.prepareStatement("SELECT 1 FROM `user_forum_badge` WHERE `user_id`=? AND `badge`=?")) {
					statement.setInt(1, p.getOwner());
					statement.setInt(2, 9);
					try(ResultSet result = statement.executeQuery()) {
						if (!result.next()) {
							DatabaseConnection.getDatabase().executeUpdate("INSERT INTO `user_forum_badge`(`user_id`, `username`, `badge_name`, `badge`) VALUES('" + p.getOwner() + "', '" + p.getUsername() + "', 'subscriber', '9')");
						}
					}
			...
			if(p.getDaysSubscriptionLeft() > 0) {
				if(removeItem(p, i.getID(), 1)) {
					...
					try (Connection connection = DatabaseConnection.getDatabase().getConnection();
						...
						try(ResultSet result = statement.executeQuery()) {
							if (!result.next()) {
								DatabaseConnection.getDatabase().executeUpdate("INSERT INTO `user_forum_badge`(`user_id`, `username`, `badge_name`, `badge`) VALUES('" + p.getOwner() + "', '" + p.getUsername() + "', 'premium subscriber', '8')");
							}
						}
					} catch (SQLException e) {
						e.printStackTrace();
		...
		case COMBAT_TOKEN:
			if(removeItem(p, i.getID(), 1)) {
				p.message("You hand over your " + i.getDef().getName().toLowerCase());
				subTime = (86400 * 30);
				if(p.combatSubscriberSubDaysLeft() > 0) {
					p.setCombatSubExpires(p.getCombatSubExpires() + subTime);
				} else {
					p.setCombatSubExpires(now + subTime);
				}
				DatabaseConnection.getDatabase().executeUpdate("UPDATE `" + Constants.GameServer.MYSQL_TABLE_PREFIX + "players` SET combat_expires='"+ p.getCombatSubExpires()  +"' WHERE `id`='" + p.getDatabaseID() + "'");
				DatabaseConnection.getDatabase().executeUpdate("UPDATE `users` SET combat_time="+ p.getCombatSubExpires()  +" WHERE `id`='" + p.getOwner() + "'");
				if(n != null)
					npcTalk(p, n, "Thank you! Your membership will last for " + p.combatSubscriberSubDaysLeft() + " days");
				p.message("Your membership is now active!");	
			} else {
				p.message("You don't have any combat tokens");
			}
	...
	public boolean blockInvAction(Item item, Player player) {
		if(item.getID() == GOLD_TOKEN || item.getID() == PREMIUM_TOKEN || item.getID() == COMBAT_TOKEN) {
			return true;
		}
		return false;
	}
	...
	public boolean blockInvUseOnItem(Player player, Item item1, Item item2) {
		if(item1.getID() == GOLD_TOKEN && item2.getID() == PREMIUM_TOKEN || item2.getID() == GOLD_TOKEN && item1.getID() == PREMIUM_TOKEN) {
			return true;
		}
		return false;
	}
	...
	public void onInvUseOnItem(Player player, Item item1, Item item2) {
		if(item1.getID() == GOLD_TOKEN && item2.getID() == PREMIUM_TOKEN || item2.getID() == GOLD_TOKEN && item1.getID() == PREMIUM_TOKEN) {
			int menu = showMenu(player, "Combine my tokens for a combat token.", "Cancel.");
			if(menu == 0) {
				removeItem(player, GOLD_TOKEN, 1);
				removeItem(player, PREMIUM_TOKEN, 1);
				message(player, 1300, "You fuse the tokens together...");
				player.message("A purple combat token is added to your inventory.");
				addItem(player, COMBAT_TOKEN, 1);
			} else if(menu == 1) {
				player.message("You decide not to combine these tokens.");
			}
		}	
	}
}